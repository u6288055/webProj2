...
function getConferenceResponses()
{
    ...
    //let APi_key = "69928a5eb89947c9893356b5f1536df5";
    let eiei=" ";
    console.log(query);
    connection.query(`SELECT * FROM product where Pname =${qurry}`, function(err ,qu)
    {
        console.log(qu);
    
    //let query="pizza";
    console.log(query+"eiei");
    let rooturl="/result/q="+query;//Request URL
    fetch(rooturl)
    .then((res)=> res.json()) // Get JSON from the response
    .then((data)=> {//console.log(data);
    
    document.querySelector("#output").innerHTML+="<h1>result</h1><ul>";
    data.results.forEach(each => {
         eiei += `<li> ${each.Pname}:<br></li>`
         
         
    })
    
    document.getElementById('output').innerHTML+= eiei+"</ul>";
    })
...
route.put('/user',function(req,res)
{
    ...
    connection.query("UPDATE user SET? WHERE UID=?",[user,UID],function(error,results){
        if(error) throw error;
        return res.send({error:false,data:results.affectedRows,message:'UID has been updated successfully.'}
        )}
        );
    });
...
route.put('/product',function(req,res)
{
    ...
    connection.query("UPDATE product SET? WHERE PID=?",[product,PID],function(error,results){
        if(error) throw error;
        return res.send({error:false,data:results.affectedRows,message:'PID has been updated successfully.'}
        )}
        );
    });
...
route.get('/result',function(req, res, q)
{
